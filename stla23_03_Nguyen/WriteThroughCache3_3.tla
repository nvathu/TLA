------------------ MODULE WriteThroughCache3_3 ------------------
EXTENDS Naturals, Sequences, MemoryInterface 
VARIABLES wmem, ctl, buf, cache, memQ, rdQ
CONSTANT QLen
...
Coherence == \A p, q \in Proc, a \in Adr : 
                (NoVal \notin {cache[p][a], cache[q][a]})
                      => (cache[p][a]=cache[q][a]) 
--------------------------------------------------------------
Req(p) == M!Req(p) /\ UNCHANGED <<cache, memQ,rdQ>> 
Rsp(p) == M!Rsp(p) /\ UNCHANGED <<cache, memQ,rdQ>> 
...
RdMiss(p) ==  /\ (ctl[p] = "busy") /\ (buf[p].op = "Rd") 
              ...
              /\ UNCHANGED <<memInt, wmem, buf, cache,rdQ>>
...
DoRd(p) == 
  ...
  /\ UNCHANGED <<memInt, wmem, cache, memQ,rdQ>> 
...
MemQWr == LET r == Head(memQ)[2] 
          IN  /\ (memQ # << >>)  /\   (r.op = "Wr") 
              ...
              /\ memQ' = Tail(memQ) 
              /\ UNCHANGED <<memInt, buf, ctl, cache,rdQ>> 
...
MemQRd == 
  ...
  IN  /\ (memQ # << >> ) /\ (r.op = "Rd")
      /\ memQ' = Tail(memQ)
      /\ rdQ' = Append(rdQ,vmem[r.adr])  
      /\ cache' = [cache EXCEPT ![p][r.adr] = Head(rdQ)]
      /\ UNCHANGED <<memInt, wmem, buf, ctl,rdQ>>
      

Evict(p,a) == /\ (ctl[p] = "waiting") => (buf[p].adr # a) 
              /\ cache' = [cache EXCEPT ![p][a] = NoVal]
              /\ UNCHANGED <<memInt, wmem, buf, ctl, memQ,rdQ>> 
...
Spec == 
  Init /\ [][Next]_<<memInt, wmem, buf, ctl, cache, memQ,rdQ>>
--------------------------------------------------------------
